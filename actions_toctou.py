...
def create_or_update_file_executable(fork_repo, fork_branch, file_content, file_path):
    """
    Create or update a file in a GitHub repository with the executable bit set
    (mode 100755), using the Git Data API.
    ...
    ref_url = f"https://api.github.com/repos/{fork_repo}/git/ref/heads/{fork_branch}"
    ref_resp = requests.get(ref_url, headers=AUTH_HEADER)
    if ref_resp.status_code != 200:
        print(f"Failed to get ref for branch {fork_branch}.")
        print("Response:", ref_resp.json())
        return
    branch_sha = ref_resp.json()["object"]["sha"]

    commit_url = f"https://api.github.com/repos/{fork_repo}/git/commits/{branch_sha}"
    commit_resp = requests.get(commit_url, headers=AUTH_HEADER)
    if commit_resp.status_code != 200:
        print(f"Failed to get commit {branch_sha}.")
        print("Response:", commit_resp.json())
        return
    base_tree_sha = commit_resp.json()["tree"]["sha"]

    blob_url = f"https://api.github.com/repos/{fork_repo}/git/blobs"
    blob_data = {
        "content": base64.b64encode(file_content).decode(),
        "encoding": "base64",
    }
    blob_resp = requests.post(blob_url, headers=AUTH_HEADER, json=blob_data)
    if blob_resp.status_code not in [200, 201]:
        print("Failed to create blob.")
        print("Response:", blob_resp.json())
        return
    blob_sha = blob_resp.json()["sha"]

    tree_url = f"https://api.github.com/repos/{fork_repo}/git/trees"
    tree_data = {
    ...
    tree_resp = requests.post(tree_url, headers=AUTH_HEADER, json=tree_data)
    if tree_resp.status_code not in [200, 201]:
        print("Failed to create tree.")
        print("Response:", tree_resp.json())
        return
    new_tree_sha = tree_resp.json()["sha"]

    new_commit_url = f"https://api.github.com/repos/{fork_repo}/git/commits"
    new_commit_data = {
        "message": "[skip ci] update",
        "tree": new_tree_sha,
        "parents": [branch_sha],
    }
    new_commit_resp = requests.post(new_commit_url, headers=AUTH_HEADER, json=new_commit_data)
    if new_commit_resp.status_code not in [200, 201]:
        print("Failed to create commit.")
        print("Response:", new_commit_resp.json())
        return
    new_commit_sha = new_commit_resp.json()["sha"]

    update_ref_url = f"https://api.github.com/repos/{fork_repo}/git/refs/heads/{fork_branch}"
    update_ref_resp = requests.patch(
        update_ref_url, headers=AUTH_HEADER, json={"sha": new_commit_sha}
    )
    if update_ref_resp.status_code == 200:
        print(f"{file_path} file created or updated (executable) in the fork repository.")
    else:
        print(f"Failed to update ref for {fork_branch}.")
        print("Response:", update_ref_resp.json())


def create_or_update_file(fork_repo, fork_branch, file_content, file_path, file_sha, executable=False):
    """
    ...
    executable (bool): If True, set the file mode to 100755 (executable) via the Git Data API.
    ...
    if executable:
        create_or_update_file_executable(fork_repo, fork_branch, file_content, file_path)
        return
...
def main(target_pr, repo, fork_repo, fork_branch, search_string, mode, update_file, update_path, executable=False):
    """
    ...
    search_string (str): Regex pattern to match against issue comments or labels.
    ...
    executable (bool): If True, set the uploaded file mode to 100755 on GitHub.
    ...
    if mode == 'comment':
        while True:
            ...
            if issue_comments:
                ...
                if re.search(search_string, most_recent_comment):
                    print(f"Most recent comment matches '{search_string}'. Proceeding to create {update_path} file.")
                  
                    create_or_update_file(fork_repo, fork_branch, file_contents, update_path, file_sha, executable)
                    break
                else:
                    print(f"Most recent comment does not match '{search_string}'. Retrying in 500ms...")
            else:
        ...
        while True:
            ...
            if any(re.search(search_string, label) for label in labels):
                print(f"Label matching '{search_string}' found. Proceeding to create {update_path}.")
                create_or_update_file(fork_repo, fork_branch, file_contents, update_path, file_sha, executable)
                break
            else:
                print(f"Label matching '{search_string}' not found. Retrying in 500ms...")
                time.sleep(.50)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--search-string", type=str, required=False, default="", help="Regex pattern to match against issue comments or labels.")
    ...
    parser.add_argument("-x", "--executable", action="store_true", help="Set the executable bit (mode 100755) on the uploaded file in GitHub.")
    ...
    main(args.target_pr, args.repo, args.fork_repo, args.fork_branch, args.search_string, args.mode, args.update_file, args.update_path, args.executable)
